...
settings.fllNoiseBandwidth       = 1.56392;%1.5;%7;      %[Hz]
% Parameters for 2nd-order FLL aided 3rd-order PLL
d_pll_w0p = settings.pllNoiseBandwidth / 0.7845; 
d_pll_w0f = settings.fllNoiseBandwidth / 0.53; 
...
trackResults(channelNr).realTimeFreq = carrFreq;
...
%% Generate the carrier frequency to mix the signal to baseband -----------
            ...
            % Get the argument to sin/cos functions
            trigarg = ((              carrFreq               * 2.0 * pi) .* time) + remCarrPhase;
            trigarg_f = (((carrFreq+(2/(3*timeCoherent/1000))) * 2.0 * pi) .* time) + remCarrPhase;
            trigarg_s = (((carrFreq-(2/(3*timeCoherent/1000))) * 2.0 * pi) .* time) + remCarrPhase;
            remCarrPhase = rem(trigarg(blksize+1), (2 * pi));
            ...
            carrSin = sin(trigarg(1:blksize));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            carrCos_f = cos(trigarg_f(1:blksize));
            carrSin_f = sin(trigarg_f(1:blksize));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            carrCos_s = cos(trigarg_s(1:blksize));
            carrSin_s = sin(trigarg_s(1:blksize));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Generate the six standard accumulated values ---------------------------
            ...
            iBasebandSignal = carrSin .* rawSignal;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            qBasebandSignal_f = carrCos_f .* rawSignal;
            iBasebandSignal_f = carrSin_f .* rawSignal;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            qBasebandSignal_s = carrCos_s .* rawSignal;
            iBasebandSignal_s = carrSin_s .* rawSignal;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            % Now get early, late, and prompt values for each
   %         I_E_pm1 = sum(earlyCode  .* iBasebandSignal);
   %         Q_E_pm1 = sum(earlyCode  .* qBasebandSignal);
            I_P_pm1 = sum(promptCode .* iBasebandSignal);
            Q_P_pm1 = sum(promptCode .* qBasebandSignal);
   %         I_L_pm1 = sum(lateCode   .* iBasebandSignal);
   %         Q_L_pm1 = sum(lateCode   .* qBasebandSignal); 
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            I_P_fm1 = sum(promptCode .* iBasebandSignal_f);
            Q_P_fm1 = sum(promptCode .* qBasebandSignal_f);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            I_P_sm1 = sum(promptCode .* iBasebandSignal_s);
            Q_P_sm1 = sum(promptCode .* qBasebandSignal_s);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %              I_E_pm2 = I_E_pm2 + I_E_pm1;
  %              Q_E_pm2 = Q_E_pm2 + Q_E_pm1;
                I_P_pm2 = I_P_pm2 + I_P_pm1;
                Q_P_pm2 = Q_P_pm2 + Q_P_pm1;
  %              I_L_pm2 = I_L_pm2 + I_L_pm1;
  %              Q_L_pm2 = Q_L_pm2 + Q_L_pm1;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%
                I_P_fm2 = I_P_fm2 + I_P_fm1;
                Q_P_fm2 = Q_P_fm2 + Q_P_fm1;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%
                I_P_sm2 = I_P_sm2 + I_P_sm1;
                Q_P_sm2 = Q_P_sm2 + Q_P_sm1;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   %                 Q_L_pm2 = 0;
     ...
     %                       I_E_p = I_E_p / timeCoherent;
     %                       Q_E_p = Q_E_p / timeCoherent;
                            I_P_p = I_P_p / timeCoherent;
                            Q_P_p = Q_P_p / timeCoherent;
     %                       I_L_p = I_L_p / timeCoherent;
     %                       Q_L_p = Q_L_p / timeCoherent;
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            I_P_f = I_P_f / timeCoherent;
                            Q_P_f = Q_P_f / timeCoherent;
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            I_P_s = I_P_s / timeCoherent;
                            Q_P_s = Q_P_s / timeCoherent;
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            Ap   = sqrt(I_P_p^2 + Q_P_p^2);
                            Af   = sqrt(I_P_f^2 + Q_P_f^2);
                            As   = sqrt(I_P_s^2 + Q_P_s^2);
                            Fe   = (2/(3*timeCoherent/1000))*(As-Af)/(2*(As-2*cos(pi*(2/(3*timeCoherent/1000))*Ap+Af)));
                            trackResults.realTimeFreq = trackResults.realTimeFreq + Fe;
%% Find PLL error and update carrier NCO ----------------------------------

                            % Implement carrier loop discriminator (phase detector)
                            %PLL_discriminator = atan2(Q_P2 , I_P2) / (2.0 * pi);
                            PLL_discriminator = atan(Q_P_p / I_P_p) / (2.0 * pi);
            
                            dot   = old_I_P * I_P_p + old_Q_P * Q_P_p;
                            cross = Q_P_p * old_I_P - I_P_p * old_Q_P; 
...
%% Find DLL error and update code NCO -------------------------------------
%% no dll loop----------------------------------------------
     %                       codeError = (sqrt(I_E_p * I_E_p + Q_E_p * Q_E_p) - sqrt(I_L_p * I_L_p + Q_L_p * Q_L_p)) / ...
...
%% Record various measures to show in postprocessing ----------------------
            % Record sample number (based on 8bit samples)
                       %     trackResults(channelNr).absoluteSample((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = ftell(fid);

                    %        trackResults(channelNr).dllDiscr((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)       = codeError;
                    %        trackResults(channelNr).dllDiscrFilt((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)   = codeNco;
                            trackResults(channelNr).pllDiscr((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)       = PLL_discriminator;
                            trackResults(channelNr).pllDiscrFilt((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)   = carrNco;%trackResults.realTimeFreq - settings.IF - 3817.75;
                            trackResults(channelNr).fllDiscr((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)       = FLL_discriminator;
                            trackResults(channelNr).fll_cross((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)      = cross;
                            trackResults(channelNr).fll_dot((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB)        = dot;

                    %        trackResults(channelNr).I_E((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = I_E_p;
                            trackResults(channelNr).I_P((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = I_P_p;
                    %        trackResults(channelNr).I_L((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = I_L_p;
                    %        trackResults(channelNr).Q_E((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = Q_E_p;
                            trackResults(channelNr).Q_P((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = Q_P_p;
                    %        trackResults(channelNr).Q_L((originalAddress-1)*timeCoherent+1+AddressB : originalAddress*timeCoherent+AddressB) = Q_L_p;
                            estimation              =             20;
                            controlVariable         =   timeCoherent/estimation;
